...
export default function Visualizer({ state, theme: customTheme }: VisualizerProps) {
  ...
  // Dynamic theme colors integrated with active theme preset
  const getTheme = () => {
    const pColor = customTheme ? customTheme.primaryHex : "rgba(139, 92, 246, 1)";
    const sColor = customTheme ? customTheme.secondaryHex : "rgba(236, 72, 153, 1)";
    ...
    switch (state) {
      case "listening": 
        return { 
          color: pColor, 
          glow: "shadow-primary", 
          border: customTheme?.activeBorder || "border-violet-400" 
        };
      case "processing": 
        return { 
      ...
      default: 
        return { 
          color: customTheme ? customTheme.primaryHex : "rgba(6, 182, 212, 0.8)", 
          glow: "shadow-cyan-500/40", 
          border: customTheme?.activeBorder || "border-cyan-500/50" 
        };
    }
  };
