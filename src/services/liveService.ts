...
import { getApiKey, getZoyaVoice } from "./apiKeyService";
import { setupSpeakerAudioChain, phoneWakeLock } from "./speakerService";
...
export class LiveSessionManager {
  ...
  async start() {
    try {
      ...
      // Re-initialize GoogleGenAI with fresh API key in case user updated settings
      this.ai = new GoogleGenAI({ apiKey: getApiKey() });
      ...
      // Get Microphone with headphone/earbuds & distance optimizations
  ...
  private playAudioChunk(base64Data: string) {
    ...
    try {
      ...
      setupSpeakerAudioChain(this.playbackContext, source);
